...
class Pointer10k(data.Dataset):
  num_classes = 1
  num_joints = 3
  default_resolution = [512, 512]
  mean = np.array([0.462215, 0.429851, 0.388770],
                   dtype=np.float32).reshape(1, 1, 3)
  std  = np.array([0.267407, 0.262841, 0.271538],
                   dtype=np.float32).reshape(1, 1, 3)
  flip_idx = [[1, 2]]
  def __init__(self, opt, split):
    super(Pointer10k, self).__init__()
    self.edges = [[0, 1], [1, 2]]
    
    self.acc_idxs = [1, 2, 3]
    self.data_dir = os.path.join(opt.data_dir, 'pointer_10k')
    self.img_dir = os.path.join(self.data_dir, 'images'.format(split))
    if split == 'test':
      self.annot_path = os.path.join(
          self.data_dir, 'annotations', 
          'ann_{}_pointer.json').format(split)
    else:
      self.annot_path = os.path.join(
        self.data_dir, 'annotations', 
        'ann_{}_pointer.json').format(split)
