...
class Pointer512(data.Dataset):
  num_classes = 1
  num_joints = 6
  default_resolution = [512, 512]
  mean = np.array([0.536199, 0.511449, 0.486500],
                   dtype=np.float32).reshape(1, 1, 3)
  std  = np.array([0.194819, 0.197950, 0.206447],
                   dtype=np.float32).reshape(1, 1, 3)
  flip_idx = [[0, 1], [2, 3], [4, 5]]
  def __init__(self, opt, split):
    super(Pointer512, self).__init__()
    self.edges = [[0, 1], [2, 3], [4, 5]]
    
    self.acc_idxs = [1, 2, 3, 4, 5, 6]
    self.data_dir = os.path.join(opt.data_dir, 'pointer512')
    self.img_dir = os.path.join(self.data_dir, 'images'.format(split))
    if split == 'test':
      self.annot_path = os.path.join(
          self.data_dir, 'annotations', 
          'Pointer512_{}.json').format(split)
    else:
      self.annot_path = os.path.join(
        self.data_dir, 'annotations', 
        'Pointer512_{}.json').format(split)
