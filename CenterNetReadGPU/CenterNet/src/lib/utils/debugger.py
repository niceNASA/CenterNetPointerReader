...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 1 or dataset == 'pointer512':
        self.names = pointer512_class
        self.names = ['p']
        self.num_class = 1
        self.num_joints = 6
        self.edges = [[0, 1], [2, 3], [4, 5]]
        self.ec = [(255, 0, 0), (0, 0, 255), (255, 0, 0)]
        self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255), (255, 0, 0), (0, 0, 255), (255, 0, 255)]
  ...
  def save_img(self, imgId='default', path='cache/debug/test/'):
    #now = datetime.now()
    cv2.imwrite(path + '{}.png'.format(imgId), self.imgs[imgId])

  def save_img_upload(self, imgId='default', path='D:\\workspace\\pointerHandler\\pointerHandler\\upload\\'):
    #now = datetime.now()
    cv2.imwrite(path + '{}.png'.format('multi_pose'), self.imgs[imgId])
    
  def save_all_imgs(self, path='D:\\workspace\\pointerHandler\\pointerHandler\\upload', prefix='', genID=False):
    if genID:
      try:
    ...
    for i, v in self.imgs.items():
      ...
      print(path + '/{}{}.png'.format(prefix, i))
